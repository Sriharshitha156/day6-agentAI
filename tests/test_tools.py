...
def test_fairness_check():
    profile_a = parse_resume(SAMPLE_RESUME)
    profile_b = parse_resume(SAMPLE_RESUME.replace("Test Candidate", "Other Candidate"))
...
def test_open_source_detection():
    ...
    profile = parse_resume(f"# OSS Dev\n## Skills\nPython\n## Experience\n- {text}")
    scorecard = score_candidate(profile, RUBRIC)
...
def test_oss_not_false_positive():
    ...
    profile = parse_resume(f"# Normal Dev\n## Skills\nPython\n## Experience\n- Deployed {text}")
    scorecard = score_candidate(profile, RUBRIC)
    python_score = [cs for cs in scorecard.criterion_scores if "Python" in cs.criterion][0]
